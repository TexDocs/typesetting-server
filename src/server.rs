...
pub fn start_server(socket_addr: &'static str) {
	let mut core = Core::new().unwrap();
	let handle = core.handle();
	// bind to the server
	let server = Server::bind(socket_addr, &handle).unwrap();
...
fn spawn_future<F, I, E>(f: F, desc: &'static str, handle: &Handle)
	where F: Future<Item = I, Error = E> + 'static,
	      E: Debug
{
	handle.spawn(f.map_err(move |e| trace!("{}: '{:?}'", desc, e))
	              .map(move |_| trace!("{}: Finished.", desc)));
}
